@php
    ...
    $isStaffSelfSettings = auth()->check() && (auth()->user()->isFaculty() || auth()->user()->isSectionHead());
@endphp
<div class="p-5 sm:p-8 flex flex-col flex-1 min-h-0">
    <div class="flex flex-col flex-1 min-h-0">
        ...
        @auth
            @if($isStaffSelfSettings)
                <p class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4 px-4">Settings</p>
                <nav class="space-y-2 mb-6 sm:mb-8">
                    <x-dashboard.nav-link :href="route('systemsettings')" :active="$r === 'systemsettings'">
                        <svg class="w-5 h-5 {{ $r === 'systemsettings' ? 'text-aps-green' : 'text-slate-400 group-hover:text-aps-green' }} shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg>
                        My account
            ...
            @unless(auth()->user()->isFaculty())
                ...
                @if(auth()->user()->isSectionHead() || auth()->user()->can('sectionheads.manage') || auth()->user()->can('faculty.view') || auth()->user()->canAccessObservations())
