        ...
        <div id="observee-card-{{ $obsUser->id }}" class="bg-white p-5 sm:p-6 lg:p-8 xl:p-10 rounded-2xl sm:rounded-3xl border border-slate-200 shadow-sm hover:shadow-2xl transition-all scroll-mt-24 sm:scroll-mt-28 group min-w-0">
            ...
            @if ($summary && (int) $summary->observation_count > 0)
                <div class="mt-6 grid grid-cols-2 gap-3 sm:mt-8 sm:gap-4">
                    <div class="min-w-0 rounded-2xl border border-slate-100 bg-slate-50/80 px-4 py-4 sm:rounded-xl sm:px-6 sm:py-5">
                    ...
                    <div class="min-w-0 rounded-2xl border border-slate-100 bg-slate-50/80 px-4 py-4 sm:rounded-xl sm:px-6 sm:py-5">
                        <p class="text-[9px] font-black uppercase tracking-[0.2em] text-slate-400 sm:text-[10px] sm:tracking-[0.25em]">Visits</p>
                        <p class="mt-1.5 text-2xl font-black text-slate-800 sm:mt-2 sm:text-3xl">{{ (int) $summary->observation_count }}</p>
                    </div>
                </div>
            @endif
            @if ($records->isNotEmpty())
                <details class="js-observation-records-details mt-6 overflow-hidden rounded-2xl border border-slate-100 bg-slate-50/50 sm:mt-8 sm:rounded-xl">
                    <summary class="flex min-h-12 cursor-pointer list-none select-none items-center justify-between gap-3 px-4 py-3 text-[11px] font-black uppercase tracking-[0.18em] text-slate-500 active:bg-slate-100/60 sm:px-6 sm:py-4 sm:text-xs sm:tracking-[0.2em]">
                        <span class="wrap:anywhere">Observation records</span>
                        <span class="inline-flex shrink-0 items-center gap-2">
                            <span class="inline-flex min-h-12 min-w-12 items-center justify-center rounded-full bg-slate-200/90 px-2 text-[10px] font-black text-slate-700">{{ $records->count() }}</span>
                                ...
                                <div class="space-y-3 sm:space-y-0 sm:flex sm:items-start sm:justify-between sm:gap-4">
                                    <div class="min-w-0 flex-1 space-y-2.5 sm:space-y-2">
                                        <div class="flex items-baseline justify-between gap-2 sm:justify-start sm:gap-3">
                                            <p class="text-sm font-black leading-tight text-slate-800 wrap:anywhere">{{ $rec->created_at->format('M j, Y') }}</p>
                                        ...
                                        <p class="text-xs font-semibold leading-snug text-slate-500 wrap:anywhere">
                                        ...
                                        @if ($rec->observation_wing || $rec->observation_department)
                                            <div class="flex flex-wrap gap-1.5 pt-0.5">
                                                @if ($rec->observation_wing)
                                                    <span class="inline-block max-w-full rounded-lg bg-slate-100 px-2 py-1 text-[10px] font-black uppercase leading-relaxed tracking-widest text-slate-700 wrap:anywhere">{{ $rec->observation_wing->label() }}</span>
                                                @endif
                                                @if ($rec->observation_department)
                                                    <span class="inline-block max-w-full rounded-lg bg-emerald-50 px-2 py-1 text-[10px] font-black uppercase leading-relaxed tracking-widest text-emerald-800 wrap:anywhere ring-1 ring-emerald-100/80">{{ $rec->observation_department->label() }}</span>
                                                @endif
                                            </div>
                                        @endif
                                        @if ($rec->notes)
                                            <p class="line-clamp-4 text-xs leading-relaxed text-slate-600 sm:line-clamp-3 wrap:anywhere">{{ $rec->notes }}</p>
                                        @endif
                                    </div>
                                    <div class="flex w-full flex-col gap-2 sm:w-auto sm:shrink-0 sm:items-stretch sm:pl-0">
                                        ...
                                        @if (auth()->user()?->canEditObservation($rec))
                                            ...
                                            <button type="button" onclick="openEditObservation(@js($editPayload))" class="inline-flex w-full min-h-11 items-center justify-center gap-1.5 rounded-xl border border-emerald-200/90 bg-emerald-50/80 px-4 text-[11px] font-black uppercase tracking-widest text-aps-green transition touch-manipulation active:bg-emerald-100/80 sm:min-h-0 sm:justify-center sm:gap-1.5 sm:border sm:border-slate-200/90 sm:bg-white sm:px-3.5 sm:py-2 sm:text-[10px] sm:font-black sm:uppercase sm:tracking-widest sm:text-emerald-800 sm:shadow-sm sm:ring-1 sm:ring-emerald-100/80 sm:hover:border-emerald-200 sm:hover:bg-emerald-50/80 sm:hover:text-aps-green sm:hover:shadow sm:active:bg-emerald-100/50">
            ...
            <div class="mt-10 pt-8 border-t border-slate-50">
                ...
                ]))" class="w-full py-5 bg-slate-900 text-white rounded-xl font-black text-xs uppercase tracking-[0.2em] shadow-xl hover:bg-aps-green transition-all">
                    Open Audit Portal
                </button>
            </div>
        </div>
    @empty
        <div class="col-span-full rounded-xl border border-slate-200 bg-white p-12 text-center text-slate-500 font-semibold">
